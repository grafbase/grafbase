...
impl<T: LegacyInputType> LegacyInputType for VecDeque<T> {
    ...
    fn parse(value: Option<Value>) -> InputValueResult<Self> {
        match value.unwrap_or_default() {
            ...
            value => Ok({
                ...
                result.push_back(LegacyInputType::parse(Some(value)).map_err(InputValueError::propagate)?);
                result
            }),
        }
    }
...
impl<T: LegacyOutputType> LegacyOutputType for VecDeque<T> {
    fn type_name() -> Cow<'static, str> {
        Cow::Owned(format!("[{}]", T::qualified_type_name()))
    }
