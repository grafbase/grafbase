...
pub enum DynamoMutationResolver {
    ...
    ///     - Store it into the ResolverContext
    ...
    async fn resolve(
        ...
        match self {
            ...
            DynamoMutationResolver::CreateNode { input, ty } => {
                ...
                // We do store the ID generated to the ResolverContext.
                if let Some(resolver_id) = resolver_ctx.resolver_id {
                    ctx.resolver_data_insert(
                ...
                for (_, edge) in edges {
                    for mut inner_edge in edge {
                        // For Vertices we do not want to have the Type GSI to be available.
                        // We do not need it.
                        inner_edge.remove("__gsi1pk");
                        inner_edge.remove("__gsi1sk");
