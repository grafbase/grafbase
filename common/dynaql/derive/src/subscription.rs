...
use quote::quote;
use syn::{
    ext::IdentExt, Block, Error, FnArg, ImplItem, ItemImpl, Pat, ReturnType, Type, TypeImplTrait, TypeParamBound,
    TypeReference,
};
                ...
                let stream = #crate_name::futures_util::stream::StreamExt::then(stream, {
                    ...
                    move |msg| {
                        ...
                        async move {
                                ...
                                */
                                ...
                                let mut resp = match query_env.extensions.resolve(ri, &mut resolve_fut).await {
                                    Ok(Some(value)) => {
                                        ...
                                        let container_id = response.insert_node(container);
                                    ...
                                    Ok(None) => {
                                        let mut container = #crate_name::graph_entities::ResponseContainer::new_container();
                                        let response = &mut *ctx.response_graph.write().await;
                                        let primitive_node_id = response.insert_node(Box::new(#crate_name::graph_entities::ResponsePrimitive::default()));
                            ...
                            ::std::result::Result::Ok(query_env.extensions.execute(query_env.operation_name.as_deref(), &query_env.operation, &mut execute_fut).await)
                        }
                    }
                });
