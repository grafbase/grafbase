...
use crate::{
    ...
    rules::{
        mongodb_directive::{model_directive::types, CreateTypeContext},
...
pub(crate) fn create(
    ...
    let input_type_name = types::input::register_input(visitor_ctx, create_ctx);

    mutation.args.insert(
        INPUT_ARG_INPUT.to_string(),
        MetaInputValue::new(INPUT_ARG_INPUT, format!("{input_type_name}!")),
    );
    ...
    visitor_ctx.mutations.push(mutation);
}
