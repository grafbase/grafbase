        ...
        if_chain! {
            ...
            then {
                ...
                for (field, _) in object.fields.iter().filter_map(|field| is_modelized_node(&ctx.types, &field.node.ty.node).map(|ty| (field, ty))) {
                    let name = field.node.directives.iter().find(|directive| directive.node.name.node == RELATION_DIRECTIVE).and_then(|dir| dir.node.get_argument("name")).and_then(|name| match &name.node {
