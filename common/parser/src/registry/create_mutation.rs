...
/*
/// Process each field depeding if it's a relation or not.
fn process_field_to_input_field<'a>(
    ctx: &mut VisitorContext<'a>,
    field: &Positioned<FieldDefinition>,
) -> (String, MetaInputValue) {
    let types = &ctx.types;
    let input_name = &field.node.name.node;
    let input_type = to_input_type(types, field.node.ty.clone().node);
    ...
    if input_name.ne(&id_field.name.node) {
        input_fields.insert(
            input_name.clone().to_string(),
            MetaInputValue {
                name: input_name.to_string(),
                description: field.node.description.clone().map(|x| x.node),
                ty: to_input_type(&ctx.types, field.node.ty.clone().node).to_string(),
