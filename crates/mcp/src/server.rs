...
use crate::{
    EngineWatcher,
    tools::{IntrospectTool, RmcpTool, RunTool, SearchTool, VerifyTool},
};
use rmcp::{
...
impl McpServer {
    pub fn new(engine: EngineWatcher<impl engine::Runtime>, enable_mutations: bool) -> anyhow::Result<Self> {
        Ok(Self(Arc::new(McpServerInner {
            ...
            tools: vec![
                Box::new(IntrospectTool::new(&engine, enable_mutations)),
                Box::new(SearchTool::new(&engine, enable_mutations)?),
                Box::new(VerifyTool::new(&engine, enable_mutations)),
                Box::new(RunTool::new(&engine, enable_mutations)),
            ],
        })))
    }
}
