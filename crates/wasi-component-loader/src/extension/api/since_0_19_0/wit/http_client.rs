...
use super::grafbase::sdk::http_types::{HttpError, HttpMethod, HttpRequest, HttpResponse, HttpVersion};
use crate::{InstanceState, http_client::send_request, resources::Headers};
...
impl HostHttpClient for InstanceState {
    async fn execute(&mut self, request: HttpRequest) -> wasmtime::Result<Result<HttpResponse, HttpError>> {
        ...
        let headers = self.resources.push(Headers::from(parts.headers))?;
