...
struct GuestFields(Vec<(String, LogField)>);
...
impl Valuable for GuestFields {
    ...
    fn visit(&self, visit: &mut dyn Visit) {
        for (key, value) in &self.0 {
            let key = Value::String(key.as_str());

            let value = match value {
                LogField::String(s) => Value::String(s.as_str()),
                LogField::I64(i) => Value::I64(*i),
                LogField::Bool(b) => Value::Bool(*b),
                LogField::U64(u) => Value::U64(*u),
                LogField::F64(f) => Value::F64(*f),
            };

            visit.visit_entry(key, value);
        }
    }
}
