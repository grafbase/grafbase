...
use crate::{cbor, extension::api::since_0_17_0::wit::schema as ws};
                ...
                for (id, subgraph_schema) in &mut subgraph_schemas {
                    if interface.exists_in_subgraph_ids.contains(id) {
                        let def = ws::TypeDefinition::Interface(ws::InterfaceDefinition {
                            ...
                            name: interface.name(),
                            interface_ids: interface.interfaces().map(|inf| inf.id.as_guid()).collect(),
                            field_ids: interface
                                .fields()
                                .filter(|field| field.exists_in_subgraph_ids.contains(id))
                                .map(|field| {
                                    subgraph_schema.field_definitions.push(ws::FieldDefinition {
                                        id: field.id.as_guid(),
                                        name: field.name(),
                                        parent_type_id: interface.id.as_guid(),
                                        arguments: field
                                            .arguments()
                                            .map(|arg| ws::InputValueDefinition {
                                                ...
                                                name: arg.name(),
                                                ty: arg.ty().into(),
                                                directives: collect_extension_directives(
                                                    arg.directives(),
                                                    extension_id,
                                                ),
                                            })
