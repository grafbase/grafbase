...
impl types::HostNatsClient for WasiState {
    async fn connect(
        &mut self,
        servers: Vec<String>,
        auth: Option<types::NatsAuth>,
    ) -> wasmtime::Result<Result<Resource<types::NatsClient>, String>> {
        ...
        let opts = match auth {
            Some(types::NatsAuth::UsernamePassword((username, password))) => opts.user_and_password(username, password),
            Some(types::NatsAuth::Token(token)) => opts.token(token),
            Some(types::NatsAuth::Credentials(ref credentials)) => match opts.credentials(credentials) {
                Ok(opts) => opts,
    ...
    async fn subscribe(
        ...
        self_: Resource<NatsClient>,
        subject: String,
        config: Option<types::NatsStreamConfig>,
    ...
    async fn request(
    ...
    ) -> wasmtime::Result<Result<types::NatsMessage, String>> {
        let client = self.get_mut(&self_)?;
        ...
        match result {
            Ok(message) => Ok(Ok(types::NatsMessage {
    ...
    async fn key_value(
        ...
        self_: Resource<NatsClient>,
        bucket: String,
    ) -> wasmtime::Result<Result<Resource<types::NatsKeyValue>, String>> {
        let client = self.get_mut(&self_)?;
...
impl types::HostNatsSubscriber for WasiState {
    async fn next(
        &mut self,
        self_: Resource<NatsSubscriber>,
    ) -> wasmtime::Result<Result<Option<types::NatsMessage>, String>> {
        let subscriber = self.get_mut(&self_)?;

        match subscriber.next().await {
            Ok(Some(message)) => Ok(Ok(Some(types::NatsMessage {
                subject: message.subject.to_string(),
                payload: message.payload.into(),
            }))),
            Ok(None) => Ok(Ok(None)),
...
impl types::HostNatsKeyValue for WasiState {
    async fn create(
        &mut self,
        self_: Resource<types::NatsKeyValue>,
        key: String,
        value: Vec<u8>,
    ...
    async fn drop(&mut self, rep: Resource<types::NatsKeyValue>) -> wasmtime::Result<()> {
        self.table.delete(rep)?;
...
impl From<types::NatsStreamDeliverPolicy> for jetstream::consumer::DeliverPolicy {
    fn from(policy: types::NatsStreamDeliverPolicy) -> Self {
        match policy {
            types::NatsStreamDeliverPolicy::All => jetstream::consumer::DeliverPolicy::All,
            types::NatsStreamDeliverPolicy::Last => jetstream::consumer::DeliverPolicy::Last,
            types::NatsStreamDeliverPolicy::New => jetstream::consumer::DeliverPolicy::New,
            types::NatsStreamDeliverPolicy::LastPerSubject => jetstream::consumer::DeliverPolicy::LastPerSubject,
            types::NatsStreamDeliverPolicy::ByStartSequence(start_sequence) => {
                jetstream::consumer::DeliverPolicy::ByStartSequence { start_sequence }
            }
            types::NatsStreamDeliverPolicy::ByStartTimeMs(ms) => {
