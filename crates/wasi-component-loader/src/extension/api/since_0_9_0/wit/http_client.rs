...
use crate::{WasiState, http_client::send_request};
...
impl HostHttpClient for WasiState {
    async fn execute(&mut self, request: HttpRequest) -> wasmtime::Result<Result<HttpResponse, HttpError>> {
        ...
        let response = send_request(http_client, request_durations, request).await;
    ...
    async fn execute_many(
        ...
        let futures = requests
            .into_iter()
            .map(|request| send_request(http_client.clone(), request_durations.clone(), request))
            .collect::<Vec<_>>();
