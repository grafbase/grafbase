...
fn invalid_id() {
    runtime().block_on(async move {
        ...
        Some(
            "At Query.echo for the extension 'echo-1.0.0' directive @echo: Found a Float value where we expected a ID scalar at path '.value'",
        )
        "#);
        ...
        insta::assert_debug_snapshot!(result.err(), @r#"
        Some(
            "At subgraph named 'a' for the extension 'echo-1.0.0' directive @meta: Found a List value where we expected a ID scalar at path '.value'",
        )
        "#);
    });
}
