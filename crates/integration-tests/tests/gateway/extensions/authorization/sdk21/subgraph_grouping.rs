    ...
    Gateway::builder()
        .with_subgraph(
            DynamicSchema::builder(
                r#"
                extend schema @link(url: "authz-21-subgraph-grouping-0.1.0", import: ["@auth"])
        ...
        .with_extension("authz-21-subgraph-grouping")
}
...
fn one_denied() {
    runtime().block_on(async move {
        let engine = builder()
            .with_toml_config(
                r#"
                [extensions.authz-21-subgraph-grouping.config]
        ...
        insta::assert_json_snapshot!(response, @r#"
        {
          ...
          "errors": [
            {
              "message": "Not authorized, denied subgraph SDK21",
              "locations": [
                {
                  "line": 1,
                  "column": 36
