...
fn nested_field() {
    runtime().block_on(async {
        let engine = Gateway::builder()
            ...
            .with_extension(Resolve::with(|args| Ok(args["code"].clone())))
            .build()
            .await;
...
fn list() {
    runtime().block_on(async {
        let engine = Gateway::builder()
            ...
            .with_extension(Resolve::with(|args| Ok(args["categories"].clone())))
            .build()
            .await;
...
fn input_object() {
    runtime().block_on(async {
        let engine = Gateway::builder()
            ...
            .with_extension(Resolve::with(|args| Ok(args["input"].clone())))
            .build()
            .await;
