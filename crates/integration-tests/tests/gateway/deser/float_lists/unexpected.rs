...
fn run(floats: serde_json::Value, query: &str) -> GraphqlResponse {
        ...
        Gateway::builder()
            ...
            .with_subgraph(
                DynamicSchema::builder(
                    ...
                    type User @key(fields: "id") {
                        id: ID!
                        floats: [Float!] @external
                        repr: JSON @requires(fields: "floats")
                    }
...
#[test]
fn invalid_floats_should_prevent_further_subgraph_request() {
    let response = run(
        json!([1, null, 3]),
        r#"
        query {
...
#[test]
fn extra_invalid_floats_should_prevent_further_subgraph_request() {
    let response = run(
        json!([1.2, null, 3.1]),
        r#"
        query {
