...
github_repo="https://github.com/grafbase/grafbase"

if [[ $# = 0 ]]; then
    grafbase_uri=$github_repo/releases/download/grafbase-$LATEST_VERSION/grafbase-$target
else
    grafbase_uri=$github_repo/releases/download/grafbase-$1/grafbase-$target
fi
...
if command --version grafbase >/dev/null; then
