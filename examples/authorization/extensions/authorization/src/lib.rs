...
use grafbase_sdk::{
    ...
    types::{AuthorizationDecisions, Configuration, DirectiveSite, ErrorResponse, QueryElements, StatusCode},
};
...
impl AuthorizationExtension for MyAuthorization {
    ...
    fn authorize_query(
        ...
        let Some(common::Token { current_user_id }) =
            ...
            return Err(ErrorResponse::new(StatusCode::INTERNAL_SERVER_ERROR));
        };
                    ...
                    for element in elements {
                        match element.site() {
                            DirectiveSite::Object(site) => match site.object_name() {
                                "User" => state.denied_ids.push(DeniedIds {
                                    query_element_id: element.id().into(),
                                    denied_ids: if let Some(ids) = authorized_user_ids.as_ref() {
                                        ids
                                    } else {
                            ...
                            DirectiveSite::FieldDefinition(site) => {
                                match (site.parent_type_name(), site.field_name()) {
                                    ("Query", "user") => {
                                        let SensibleArguments { arguments, .. } = element.arguments()?;
        ...
        .unwrap();
        subgraph_headers.set("Authorization", [scopes]);
        ...
        for element in elements {
                ...
                .find(|denied| denied.query_element_id == u32::from(element.query_element_id()))
            {
                ...
                for item in element.items() {
                    let SensibleArguments { fields, .. } = item.deserialize()?;
                    let fields = fields.unwrap();
                    if denied.denied_ids.contains(&(fields.id_as_u32().into())) {
                        let error_id = *lazy_error_id
                            .get_or_insert_with(|| builder.push_error("Not authorized: cannot access user"));
...
impl MyAuthorization {
    fn get_authorized_ids(&self, current_user_id: u32) -> Result<Vec<u32>, ErrorResponse> {
        host_io::http::execute(
            ...
            .body(serde_json::to_vec(&serde_json::json!({"current_user_id": current_user_id})).unwrap()),
        )
        .map(|response| {
            let AuthorizationResponse { authorized_users } = response.json().unwrap();
            authorized_users
        })
        .map_err(|e| ErrorResponse::new(StatusCode::UNAUTHORIZED).with_error(Error::new(e.to_string())))
    }
}
#[derive(serde::Deserialize)]
...
fn unsupported() -> ErrorResponse {
    ErrorResponse::new(StatusCode::INTERNAL_SERVER_ERROR).with_error(Error::new("Unsupported"))
}
