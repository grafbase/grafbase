...
- The `@derive` directive lets you define virtual fields on a type, so you can build a type not defined in your service from one of its keys. For example, if your Product type has a warehouse_id field referencing a location, you can declaratively define a Location type to enable joins between services:
  ...
  type Product {
      sku: String!
      warehouse_id: ID!
      warehouse: Location! @derive @is(fields: )
  }
...
- The `grafbase.graphql.object_extra_field` protocol buffers option in conjunction with `@require` and `@grpcMethod` (from the [gRPC extension](https://grafbase.com/extensions/grpc)) allows joins based on gRPC methods of arbitrary shapes, not only lookups (lookups take ids and return the entities directly).

```proto
option (grafbase.graphql.object_extra_field) =
  "parts("
...
```
  ...
  This is not very pretty due to the multi-line strings syntax in .proto files, but relatively straightforward to use. You can see examples of the `@grpcMethod` directive in action in the code generated by protoc-gen-grafbase-subgraph.
