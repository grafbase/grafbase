...
import { Mutation, TodoListDeleteDocument, TodoListFragment, TodoListUpdateDocument } from '~/graphql/schema'
import getColor from '~/utils/get-color'
import { grafbase } from '~/utils/grafbase'

const TodoList = (props: TodoListFragment) => {
  const [deletingTodoList, deleteTodoList] = createServerAction$(async (form: FormData) => {
    const vars = Object.fromEntries(form)
    await grafbase.request<Mutation>(TodoListDeleteDocument, vars)
    return redirect('/')
  })

  const [updatingTodoList, updateTodoList] = createServerAction$(async (form: FormData) => {
    const vars = Object.fromEntries(form)
  ...
  const title = () => (updatingTodoList.pending ? (updatingTodoList.input.get('title') as string) : props.title)
  let inputRef: HTMLInputElement

  return (
    <div class='space-y-4 flex-1 min-w-[300px]'>
      <div class='flex justify-between border-b-2' title={title() || ''} style={{ 'border-color': getColor(props.id) }}>
        <updateTodoList.Form>
        ...
        <div class='relative z-20'>
          <Dropdown trigger={<DotsVerticalIcon class='w-5 h-5 text-gray-400 transition hover:text-red-400' />}>
            <DropdownItem>
              <div class='group px-2 py-2 rounded-md hover:bg-emerald-200 hover:dark:bg-emerald-700'>
                <button onClick={() => setTimeout(() => inputRef.focus())} class='flex items-center w-full text-sm'>
      ...
      <div class='space-y-4'>
        <For each={todos()}>{(todo) => !!todo?.node && <TodoItem {...todo.node} />}</For>
      </div>
