...
import { Mutation, TodoDeleteDocument, TodoUpdateDocument } from '~/graphql/schema'
...
const TodoItem = (props: Props) => {
  const [deletingTodo, deleteTodo] = createServerAction$(async (form: FormData) => {
    const vars = Object.fromEntries(form)
    await grafbase.request<Mutation>(TodoDeleteDocument, vars)
    return redirect('/')
  })

  const [updatingTodo, updateTodo] = createServerAction$(async (form: FormData) => {
    const vars = Object.fromEntries(form)
  ...
  const title = () => (updatingTodo.pending ? (updatingTodo.input.get('title') as string) : props.title)
  const completed = () => (updatingTodo.pending ? updatingTodo.input.get('complete') === 'on' : props.complete)
    ...
    >
      ...
      <div class='relative'>
        <div class='flex justify-between gap-4'>
          <updateTodo.Form>
            <fieldset title={title()} disabled={updatingTodo.pending} class='flex space-x-1.5 items-center truncate'>
        ...
        <div class='flex justify-between mt-2 text-sm'>
          <div
            class={`text-xs px-1 py-0.5 rounded ${
              completed() ? 'bg-green-600 text-white' : 'bg-gray-300 dark:bg-gray-600'
            }`}
          >
