...
impl CliError {
    ...
    pub fn to_hint(&self) -> Option<String> {
        match self {
            ...
            Self::BackendApiError(ApiError::NotLoggedIn) => Some("try running 'grafbase login'".to_owned()),
            Self::BackendApiError(ApiError::RequestError) => Some("you may be using an older version of the Grafbase CLI, try updating".to_owned()),
            Self::BackendApiError(ApiError::CreateError(CreateError::Unknown)) => Some("you may be using an older version of the Grafbase CLI, try updating".to_owned()),
            Self::BackendApiError(ApiError::DeployError(DeployError::Unknown)) => Some("you may be using an older version of the Grafbase CLI, try updating".to_owned()),
            Self::BackendApiError(ApiError::CorruptCredentialsFile) => Some("try running 'grafbase login'".to_owned()),
            Self::BackendApiError(ApiError::ProjectAlreadyLinked) => Some("try running 'grafbase deploy'".to_owned()),
            Self::BackendApiError(ApiError::CorruptProjectMetadataFile) => Some("try running 'grafbase link'".to_owned()),
            Self::BackendApiError(ApiError::UnlinkedProject) => Some("try running 'grafbase link'".to_owned()),
            _ => None,
        }
    }
}
