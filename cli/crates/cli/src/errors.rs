        ...
        match self {
            ...
            Self::BackendError(BackendError::DeleteDotGrafbaseDirectory(error)) => {
                match error.kind() {
                    ...
                    ErrorKind::PermissionDenied => Some("it appears that you do not have sufficient permissions to delete the .grafbase directory, try modifying the directory's permissions".to_owned()),
                    _ => None,
                }
            }
            _ => None,
