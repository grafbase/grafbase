...
fn test_kv_integration(#[case] enabled: bool) {
    ...
    //     "test.js",
    //     r#"
    //     export default async function Resolver(_p, _x, context, _t) {
    //         const kvKey = "test";

    //         let { value } = await context.kv.get(kvKey);
    //         if (value === null) {
    //             console.info(`Key ${kvKey} doesn't exist in KV. Creating ...`);
    //             await context.kv.set(kvKey, "hello kv!");
    //         }

    //         let { value: kv_value } = await context.kv.get(kvKey);
