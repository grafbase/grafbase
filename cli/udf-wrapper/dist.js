...
var server = (0, import_http.createServer)((request, response) => {
  ...
  if (routerResponse instanceof Promise) {
    routerResponse.then((routerResponse2) => webResponseToNode(routerResponse2, response));
  } else {
...
var nodeRequestToWeb = (request) => new Request(`${DUMMY_HOST}${request.url}`, {
  method: request.method,
  // the cast here is likely required because of node fetch still being experimental
  headers: request.headers,
  body: request.method === "POST" ? import_stream2.Readable.toWeb(request) : void 0,
  duplex: "half" /* Half */
});
var webResponseToNode = (webResponse, response) => {
  webResponse.headers.forEach((value, key) => response.setHeader(key, value));
  response.statusMessage = webResponse.statusText;
  response.statusCode = webResponse.status;
  if (webResponse.body !== null) {
    import_stream2.Readable.fromWeb(webResponse.body).on("data" /* Data */, (chunk) => response.write(chunk)).on("end" /* End */, () => response.end());
  } else {
    response.end();
  }
};
