...
impl KvStoreInner for NoopKvStore {
    async fn get(&self, _name: &str, _cache_ttl: Option<Duration>) -> KvResult<Option<Bytes>> {
        Err(KvError::Kv("Not available".into()))
    }

    async fn put(&self, _name: &str, _bytes: Bytes, _expiration_ttl: Option<Duration>) -> KvResult<()> {
        Ok(())
    }
}
