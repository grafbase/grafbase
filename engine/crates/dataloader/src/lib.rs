        ...
        T: Loader<K>,
    {
        ...
        match self.loader.load(&keys).await {
            Ok(values) => {
                ...
                let typed_requests = request.get_mut(&tid).unwrap().downcast_mut::<Requests<K, T>>().unwrap();
...
impl<T, C: CacheFactory> DataLoader<T, C> {
    ...
    where
        ...
        T: Loader<K>,
    {
        let mut values = self.load_many(std::iter::once(key.clone()), spawner).await?;
