...
impl OperationDetails {
    pub fn new(
        ...
        operation: &openapiv3_1::Operation,
        ...
        let request_bodies = match &operation.request_body {
            ...
            Some(ReferenceOr::Item(request_body)) => Rc::new(RequestBody::from_openapi_3_1(request_body)),
        };

        let mut responses = Vec::new();
        if let Some(operation_responses) = &operation.responses {
            for (status_code, response) in &operation_responses.responses {
                match &response {
...
impl RequestBody {
    pub fn from_openapi_3_1(request_body: &openapiv3_1::RequestBody) -> Vec<RequestBody> {
        request_body
            .content
            .iter()
...
fn convert_status_code(code: &openapiv3_1::StatusCode) -> Result<ExpectedStatusCode, Error> {
    match code {
...
mod tests {
    ...
    fn test_status_codes(#[case] input: &str, #[case] expected: ExpectedStatusCode) {
        let status_code =
            serde_json::from_value::<openapiv3_1::StatusCode>(serde_json::Value::String(input.to_string())).unwrap();
