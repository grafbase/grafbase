...
use grafbase_engine_value::{Name, Value};
...
#[cfg(test)]
mod tests {
    use grafbase_engine_parser::Pos;
    ...
    fn serialize(input: &str) -> String {
        ...
        let variable_definition = VariableDefinition {
            ...
            var_type: Positioned::new(
                grafbase_engine_parser::types::Type::new("Bool").unwrap(),
                Pos::default(),
            ),
            directives: vec![],
    ...
    fn input_to_selections(input: &str) -> (Vec<Selection>, HashMap<Name, FragmentDefinition>) {
        let document = grafbase_engine_parser::parse_query(input).unwrap();
