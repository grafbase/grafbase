...
mod tests {
    ...
    fn should_validate_with_cache_view() {
        ...
        // act
        let document = grafbase_engine_parser::parse_query(gql_query_cached).expect("should properly parse");
        let cached_validation_result =
            check_rules(&registry, &document, None, ValidationMode::Fast).expect("should validate successfully");

        let document = grafbase_engine_parser::parse_query(gql_query_non_cached).expect("should properly parse");
