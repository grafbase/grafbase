...
use quote::quote;
use syn::{
    visit::Visit, visit_mut, visit_mut::VisitMut, Attribute, Error, Expr, ExprPath, FnArg, Ident, ImplItemFn, Lifetime,
    Lit, LitStr, Meta, Pat, PatIdent, Type, TypeGroup, TypeParamBound, TypeReference,
};
use thiserror::Error;
...
pub fn get_crate_name(internal: bool) -> TokenStream {
    if internal {
        ...
        let name = match crate_name("async-graphql") {
            ...
            Ok(FoundCrate::Itself) | Err(_) => "grafbase_engine".to_string(),
        };
