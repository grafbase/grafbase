...
use schema::{Definition, EnumWalker, InputObjectWalker, ListWrapping, ScalarType, ScalarWalker, Schema, Type};
...
impl<'a> VariableCoercionContext<'a> {
    ...
    fn coerce_input_objet(
        ...
        let ids = self
            .input_values
            .reserve_input_object(input_object.as_ref().input_field_ids.len());
        for (input_field, id) in input_object.input_fields().zip(ids) {
            match fields.swap_remove(input_field.name()) {
                None => {
                    if let Some(default_value_id) = input_field.as_ref().default_value {
                        self.input_values[id] = (input_field.id(), VariableInputValue::DefaultValue(default_value_id));
