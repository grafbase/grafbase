...
use schema::{Definition, EnumWalker, InputObjectWalker, ListWrapping, ScalarType, ScalarWalker, Type, Wrapping};
            ...
            match fields.swap_remove(input_field.name()) {
                None => {
                    if let Some(default_value_id) = input_field.as_ref().default_value {
                        self.input_values[id] = (input_field.id(), QueryInputValue::DefaultValue(default_value_id));
                        ...
                        self.input_values[id] = (input_field.id(), QueryInputValue::Undefined);
                    }
                }
                Some(value) => {
