...
impl<'de> serde::Deserializer<'de> for VariableInputValueWalker<'de> {
    ...
    {
        match self.item {
            VariableInputValue::Null | VariableInputValue::Undefined => visitor.visit_none(),
            ...
            VariableInputValue::List(ids) => {
                let mut deserializer = SeqDeserializer::new(ids.map(move |id| self.walk(&self.variables[id])));
            ...
            VariableInputValue::InputObject(ids) => {
                let mut deserializer = MapDeserializer::new(ids.filter_map(move |id| {
                    let (input_value_definition_id, value) = &self.variables[id];
            ...
            VariableInputValue::Map(ids) => {
                let mut deserializer = MapDeserializer::new(ids.map(move |id| {
                    let (key, value) = &self.variables[id];
                    let value = self.walk(value);
                    (key.as_str(), value)
                }));
                let map = visitor.visit_map(&mut deserializer)?;
    ...
    where
        V: Visitor<'de>,
    {
        if matches!(self.item, VariableInputValue::Null | VariableInputValue::Undefined) {
            visitor.visit_none()
        } else {
