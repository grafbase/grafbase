...
impl VariableInputValues {
    ...
    /// Vec to hold values as we need them to be contiguous.
    pub fn reserve_input_object(&mut self, n: usize) -> IdRange<VariableInputObjectFieldValueId> {
        let start = self.input_fields.len();
        self.input_fields.reserve(n);
        for _ in 0..n {
            self.input_fields
                .push((InputValueDefinitionId::from(0), VariableInputValue::Null));
        }
...
impl<'a> From<VariableInputValueWalker<'a>> for InputValue<'a> {
    fn from(walker: VariableInputValueWalker<'a>) -> Self {
        match walker.item {
            VariableInputValue::Undefined => InputValue::Undefined,
...
impl std::fmt::Debug for VariableInputValueWalker<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self.item {
            VariableInputValue::Undefined => write!(f, "Undefined"),
