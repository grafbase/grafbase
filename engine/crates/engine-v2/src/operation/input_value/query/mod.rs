...
impl QueryInputValues {
    ...
    /// Vec to hold values as we need them to be contiguous.
    pub fn reserve_input_object(&mut self, n: usize) -> IdRange<QueryInputObjectFieldValueId> {
        let start = self.input_fields.len();
        self.input_fields.reserve(n);
        for _ in 0..n {
            self.input_fields
                .push((InputValueDefinitionId::from(0), QueryInputValue::Null));
        }
...
impl<'a> QueryInputValueWalker<'a> {
    pub fn is_undefined(&self) -> bool {
        match self.item {
            ...
            QueryInputValue::Undefined => true,
            _ => false,
        }
    }
}
...
impl<'a> From<QueryInputValueWalker<'a>> for InputValue<'a> {
    fn from(walker: QueryInputValueWalker<'a>) -> Self {
        match walker.item {
            QueryInputValue::Undefined => InputValue::Undefined,
            ...
            QueryInputValue::Variable(id) => walker.walk(*id).into(),
        }
    }
}
...
impl std::fmt::Debug for QueryInputValueWalker<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self.item {
            QueryInputValue::Undefined => write!(f, "Undefined"),
