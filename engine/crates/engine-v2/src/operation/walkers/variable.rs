...
impl<'a> From<VariableWalker<'a>> for InputValue<'a> {
    fn from(walker: VariableWalker<'a>) -> Self {
        if let Some(id) = walker.variables[walker.item] {
            walker.walk(&walker.variables[id]).into()
        } else if let Some(id) = walker.as_ref().default_value {
            walker.walk(&walker.operation.query_input_values[id]).into()
        } else {
