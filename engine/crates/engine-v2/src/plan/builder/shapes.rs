...
use crate::{
    operation::{
        blueprint::partition::{partition_shapes, Partition},
        BoundFieldId,
    },
    plan::{
    ...
    response::{
        ...
        PolymorphicObjectShapeId, ResponseKey, Shape, Shapes,
    },
    utils::BufferPool,
};
...
impl OperationPlanBuilder<'_> {
    pub(super) fn populate_plan_shapes(&mut self) {
        ...
        self.operation_plan.plans = plans;
        self.operation_plan.shapes = shapes;

        for id in data_field_ids_with_selection_set_requiring_typename {
            self.operation_plan[id].selection_set_requires_typename = true
        }
        ...
        for (data_field_id, field_shape_ids) in field_id_to_field_shape_ids
            ...
            .into_iter()
        {
            let start = self.operation_plan.field_shape_refs.len();
            self.operation_plan
                .field_shape_refs
                .extend(field_shape_ids.into_iter().map(|(_, id)| id));
            self.operation_plan[data_field_id].shape_ids =
                IdRange::from(start..self.operation_plan.field_shape_refs.len());
        }
    }
}
...
impl<'ctx> ShapesBuilder<'ctx> {
    ...
    fn create_data_field_shape(
        ...
        FieldShape {
            expected_key: self.operation_plan.response_keys.ensure_safety(response_key),
            edge: field.key.into(),
            // FIXME: proper field id.
            id: BoundFieldId::from(0usize),
            required_field_id,
