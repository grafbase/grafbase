...
use crate::{
    operation::{
        ...
        BoundVariableDefinition, BoundVariableDefinitionId,
    },
    plan::PlanError,
...
impl OperationSolution {
    pub(super) fn solve(schema: &Schema, mut bound_operation: BoundOperation) -> PlanResult<Self> {
...
struct OperationSolutionBuilder<'a> {
    schema: &'a Schema,
    operation: OperationSolution,
...
impl OperationSolutionBuilder<'_> {
    fn build(mut self) -> PlanResult<OperationSolution> {
        self.operation
    ...
    fn populate_requirements_after_partition_generation(&mut self) -> PlanResult<()> {
        self.scalar_field_node_to_field.sort_unstable();

        fn get_scalar_field_id_for(builder: &OperationSolutionBuilder<'_>, node_ix: NodeIndex) -> Option<DataFieldId> {
            builder
        ...
        fn push_requirements(
            builder: &mut OperationSolutionBuilder<'_>,
            node_ix: NodeIndex,
            kind: Edge,
        ) -> IdRange<DataFieldRefId> {
        ...
        for (query_partition_id, query_partition_root_node_ix) in query_partition_to_node.iter().copied() {
            self.operation[query_partition_id].required_scalar_field_ids =
    ...
    fn populate_modifiers_after_partition_generation(&mut self) -> PlanResult<()> {
    ...
    fn generate_mutation_partition_order_after_partition_generation(&mut self) -> PlanResult<()> {
        ...
        fn get_query_partition_id(
            builder: &OperationSolutionBuilder<'_>,
            node_ix: NodeIndex,
        ) -> PlanResult<QueryPartitionId> {
            builder
