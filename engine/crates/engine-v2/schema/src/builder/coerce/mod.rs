...
use crate::{
    Definition, EnumDefinitionId, EnumValueDefinitionId, Graph, InputObjectDefinitionId, InputValueDefinitionId,
    ScalarDefinitionId, ScalarType, SchemaInputValue, SchemaInputValueId, SchemaInputValues, StringId, Type,
};
pub use error::*;
...
impl<'a> InputValueCoercer<'a> {
    ...
    fn coerce_enum(&mut self, enum_id: EnumDefinitionId, value: Value) -> Result<SchemaInputValue, InputValueError> {
        ...
        {
            Ok(id) => Ok(SchemaInputValue::EnumValue(EnumValueDefinitionId::from(id))),
