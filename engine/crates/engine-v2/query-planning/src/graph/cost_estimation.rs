...
pub(crate) struct CostEstimator {
    ...
    edge_to_impacted_requirement: Vec<(EdgeIndex, NodeIndex)>,
...
impl<'ctx, Op: Operation> OperationGraphBuilder<'ctx, Op> {
    ...
    pub(crate) fn build_cost_estimator(&mut self) -> crate::Result<CostEstimator> {
        let mut estimator = CostEstimator {
            edge_cost: vec![0; self.edge_cost_count()],
...
impl CostEstimator {
    ...
    // - need all requirements in all cases. Don't need a max.
    // - so distinct edgecost wouldn't work because some paths might not be taken.
    // - the "simple" solution is to go over all fields???
    ...
    //
    // Ideal goal:
    // - needs fields from 2 different subgraphs -> cost of 2
    // - need nested field from a different subgraph -> cost of 2
    // - only need fields from a single subgraph -> 1
    // - only parent -> 0
    //
    // maybe just need to compute one way to do it. doesn't need to be perfect computation.
    // - can group by origin query field, but how to know which one to reach? back to the steiner
    // tree problem...
    //
    // for a single field -> it's the min of all resolvers/providable field cost to that field.
    //  - from origin, find relevant query field, and move up in ancestry
    // for multiple fields are the origin level, same thing.
    // -> iterate recursively setting new origin and field.
    //
    // defining minimum cost of requirements is again a steiner tree problem!
    // most common cases are only root fields that are easily computed though, so we can
    // approximate that problem to the root fields.
    //
    // even for root fields, how to deal with it?
    // iterate over all edges, set of edge cost id, different cases.
    //
    // Solve it the hard way, create all combinations. if parent can provide it, cost 0.
    // otherwise combinations? yeah.
    //
    // edge -> impacted requirements computed from ancestry.
    // ancestry not really useful otherwise with the steiner tree problem.
    //
    // requirements -> incoming edge cost.
    //
    // compute cost of requirement with very naive steiner tree algorithm.
    //
    fn populate_from<Id>(&mut self, graph: &mut StableGraph<Node<Id>, Edge>) {
        ...
        for (requirement_query_field_ix, node) in graph.node_references() {
            ...
            requirement_origins.extend(
                graph
                    .edges_directed(requirement_query_field_ix, Direction::Incoming)
            ...
            while let Some(node_ix) = stack.pop() {
                for edge in graph.edges_directed(node_ix, Direction::Incoming) {
                    match edge.weight() {
                        Edge::CreateChildResolver { .. } | Edge::CanProvide { .. } => {
                            ancestry_edges.push(edge.id());

                            let node_ix = edge.source();
                            if graph[node_ix].query_depth() >= min_resolver_depth {
                                stack.push(node_ix);
                            }
                        }
                        Edge::Provides
            ...
            self.edge_to_impacted_requirement.extend(
                ancestry_edges
                    .drain(..)
                    .dedup()
                    .map(|edge| (edge, requirement_query_field_ix)),
            );
        }
    ...
    fn fixed_point_iteration<Id>(
        ...
        mut impacted_requirements: Vec<NodeIndex>,
    ) -> crate::Result<()> {
        let mut iterations = 0;
        let mut impacted_nodes = impacted_requirements
            .drain(..)
            .flat_map(|requirement_query_field_ix| {
                graph
                    .edges_directed(requirement_query_field_ix, Direction::Incoming)
                    .filter_map(|edge| match edge.weight() {
        ...
        for node_ix in impacted_nodes.drain(..).dedup() {
            let mut requirements = graph
                ...
                .filter_map(|edge| match edge.weight() {
                    Edge::Requires { origin_query_field_ix } => Some((*origin_query_field_ix, edge.target())),
                    _ => None,
                })
                .collect::<Vec<_>>();
            requirements.sort_unstable();

            for (origin_query_field_ix, requirement_query_field_indices) in
                requirements.drain(..).dedup().chunk_by(|(ix, _)| *ix).into_iter()
            {
                todo!()
            }
        }
