...
fn node_update<'a>(
    ...
    for (field, operation) in numerical_operations {
        if let Some(set) = operation.get("set") {
            update_attr.push((
                Name::new(field),
                grafbase_engine_value::ConstValue::from_json(set.clone()).expect("must parse"),
            ));
    ...
    pub(super) async fn resolve(
        ...
        match self {
            ...
            DynamoMutationResolver::UpdateNode { by, input, ty } => {
                ...
                if by_id {
                    let value: String = grafbase_engine_value::from_value(value.clone()).expect("cannot fail");
            ...
            DynamoMutationResolver::DeleteNode { by, ty } => {
                ...
                if by_id {
                    let id_to_be_deleted: String =
                        grafbase_engine_value::from_value(value.clone()).expect("cannot fail");
